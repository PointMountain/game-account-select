...
- User asked for a pre-execution preparation skill that checks the query-governance and browser runtime dependencies, auto-installs only when safe, and guides manual installation otherwise.
...
- Check Node, `gh`, `git`, ego-ops, and the configured browser skill availability.
...
- Provide a documented workflow for execution-time refresh using ego-ops-governed browser operations.
