...
game-account-skill-evaluator      评估 skill 或优化器产物是否达到使用标准，低分打回重做
game-account-community-updater    刷新社区证据快照和覆盖限制
game-account-skill-optimizer      分析执行问题、内置 Troubleshooting，并生成 skill 优化建议
...
这个结构借鉴主流工程化 skill 框架的原则：薄编排、文档即状态、验证报告可复查、每个阶段都有明确输入输出。整体设计倾向 self-evolving harness：运行 artifact 进入优化器，优化器做故障诊断和定位，评估器决定产物是否能继续进入真实流程。
...
当游戏未支持时，主筛选 skill 先调用 `game-account-skill-generator` 生成保守基线 skill，再调用 `game-account-skill-evaluator` 做质量门禁。未通过门禁或 `redo_required: true` 时，只输出阻塞问题和社区刷新建议，不直接给真实购买推荐。
...
- 是否需要更新游戏规则
- 是否需要更新平台解析模式
- evaluator 质量门禁结果、阻塞问题和是否 `redo_required`
...
5. 记录 changelog。
6. 运行目标验证脚本和 `game-account-skill-evaluator`。
7. 若低于门槛、存在阻塞问题或 `redo_required: true`，打回重做，不进入真实推荐。
...
- 说明目标文件和原因。
- 运行对应验证脚本。
- 运行 evaluator 并处理打回重做。
- 不把单次反馈无证据升级为高置信硬规则。

## Troubleshooting 与质量门禁

优化 skill 必须内置 Troubleshooting 能力。遇到耗时长、空结果、平台缺口、输出文案异常、估值误判或工具失败时，先归因到具体阶段，再决定修改主筛选、工具层、平台策略、游戏规则、社区证据、生成器、评估器或优化器自身。

评估器不只用于新增游戏 skill，也用于优化器产出的 skill 本身。低分不是提示信息，而是流程控制信号：调用方必须把产物打回重做，直到验证脚本和 evaluator 都通过。
