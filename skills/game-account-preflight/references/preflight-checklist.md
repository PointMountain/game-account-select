...
- chrome-use extension relay：优先复用真实 Chrome 的登录态和用户可见页面，不依赖 remote-debugging-port。
- web-access skill + Chrome remote debugging：`chrome-use` 不可用时的浏览器兜底。
- OCR：只有当账号资产只在图片中时需要。
...
- 缺 `chrome-use` 但 `web-access` + CDP 可用：使用 CDP 兜底，并提示可能出现一次性授权。
