...
`autopatch_safe: true` 只适合流程文档、平台顺序、输出格式和运行记录字段这类低风险改动。估值权重、社区 meta、高价值角色分层、平台解析器实现和风险扣分默认 `autopatch_safe: false`，除非用户明确要求实现并且验证脚本覆盖对应误判。
...
- 更新对应游戏的 `valuation-rules.md` 和知识表。
- 新增验证样例，确保相同误判不复发。
