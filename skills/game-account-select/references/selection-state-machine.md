...
5. 对盼之这类前端路由站点，直接接口、手写查询 URL 或静态抓取失败时，不要立刻判定平台不可读；如果浏览器 CDP 可用，必须先从 `https://www.pzds.com/gameList` 页面内自然点击目标游戏入口，验证是否能进入真实商品列表。
6. 平台页面不可读时，请用户提供截图/链接/复制文本。
7. 记录数据来源和限制，不要声称覆盖了未成功读取的平台。
...
“预算内性价比”不是“最低价优先”。预算上限内的账号应分层比较：

- 先保留足够候选：主流平台可读时，至少比较低价段、中价段和接近预算上限的账号；若少于 5 个候选，必须解释平台限制或继续补平台。
- 再看成交质量：核心资产、热门队伍、关键装备、资源余量、平台保障和绑定风险共同决定性价比。
- 低价账号只有在风险字段清楚、核心资产足够、且成型度不明显落后时，才可压过接近预算上限的成品号。
- 当 700-1000 元账号提供明显更完整的队伍、专武、资源或更低风险时，不应被 100-200 元账号仅因价格更低而压过。
