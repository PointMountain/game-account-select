...
- 游戏估值 skill：文件结构、估值规则、社区证据、风险规则、公共 evaluator、raw-artifact finalizer、交付门禁和验证样例。
...
- 游戏估值 skill 缺 fixture-independent `evaluate-listing.mjs`、确定性 finalizer、finalizer 回归、`request_provenance`、`self_improve`、`quality_gate` 或 SHA-bound `delivery_contract`。
- selector/toolkit 没有读取并验证 operation support matrix，或把 unsupported route 当成已覆盖能力。
...
- 平台或 operation 已提供上架/验号时间，但推荐、备选或排除项没有保留标准字段 `published_at` / `platform_verified_at`；或用抓取/运行时间代替来源时间。
...
- `verified_existing` 只能计为已有机制复核，不能计入本轮 `applied`；观察到运行前已有 operation/fallback 不得冒充本轮代码优化。
